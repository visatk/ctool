...
type TabId = 'generate' | 'decode' | 'hash' | 'base64';
...
const PRESETS: Preset[] = [
  ...
  { id: 'api_live', name: 'API Key (Live)', desc: 'Prefixed sk_live_ key', config: { length: 32, type: 'alphanumeric', prefix: 'sk_live_' } },
  { id: 'api_test', name: 'API Key (Test)', desc: 'Prefixed sk_test_ key', config: { length: 32, type: 'alphanumeric', prefix: 'sk_test_' } },
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-gray-200 font-sans selection:bg-emerald-500/30">
      ...
      <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${toast ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <aside className="lg:w-64 shrink-0 flex flex-col gap-6">
          <div className="mb-4 text-center lg:text-left">
            ...
            <p className="text-xs text-gray-500 mt-2">100% Client-Side. Zero Tracking.</p>
          </div>
          ...
          <nav className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 hide-scrollbar">
            ...
            <NavButton id="base64" current={activeTab} set={setActiveTab} title="Base64 Tool" icon={<path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h7" />} />
          </nav>
        </aside>

        {/* Main Workspace Content */}
        <main className="flex-1 min-w-0 flex flex-col gap-8">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-5 sm:p-8 overflow-hidden min-h-[600px]">
...
function NavButton({ id, current, set, title, icon }: { id: TabId, current: TabId, set: (i: TabId) => void, title: string, icon: React.ReactNode }) {
  ...
  return (
    ...
    >
      <svg className="w-5 h-5 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor">{icon}</svg>
      {title}
    </button>
  );
}
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-8 h-full">
      <div className="relative group">
        ...
        <div className="relative bg-gray-950 border border-gray-800 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <code className="font-mono text-emerald-400 text-lg sm:text-xl break-all w-full select-all leading-tight">{secret}</code>
          <div className="flex shrink-0 gap-2 self-end sm:self-auto">
            <button onClick={handleGenerate} className="p-3 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-colors outline-none focus:ring-2 focus:ring-emerald-500" title="Regenerate">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
        ...
        <div className={`bg-gray-950/50 p-5 rounded-xl border border-gray-800 flex flex-col gap-5 transition-opacity ${activePreset !== 'custom' ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
           <div>
            ...
            <input type="range" min="12" max="256" step="4" value={customLength} onChange={(e) => { setCustomLength(Number(e.target.value)); handleGenerate(); }} disabled={activePreset !== 'custom' || customType === 'uuid'} className="w-full h-2 bg-gray-800 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={customPrefix} onChange={(e) => { setCustomPrefix(e.target.value); handleGenerate(); }} disabled={activePreset !== 'custom' || customType === 'uuid'} placeholder="sk_live_" className="w-full bg-gray-900 border border-gray-800 rounded px-3 py-2 text-gray-200 outline-none focus:ring-1 focus:ring-emerald-500 font-mono text-sm" />
          </div>
        </div>
      </div>
    </div>
...
function JwtDecoderTool({ copy }: { copy: (t: string) => void }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col lg:flex-row gap-6 h-full">
      <div className="flex-1 flex flex-col">
        <label className="text-sm font-semibold text-gray-300 mb-2">Encoded JWT</label>
        <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="eyJhbGciOiJIUzI1Ni..." className="w-full flex-1 min-h-[200px] lg:min-h-full bg-gray-950 border border-gray-800 rounded-xl p-4 text-emerald-400/80 font-mono text-sm leading-relaxed resize-none outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent placeholder:text-gray-700" spellCheck={false} />
      </div>
      <div className="flex-1 flex flex-col gap-4">
        {error ? (
          <div className="flex-1 flex items-center justify-center bg-gray-950 border border-red-900/50 rounded-xl p-6 text-center text-red-400 font-medium">
            <div><svg className="w-10 h-10 text-red-500/50 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>{input ? error : 'Waiting for token...'}</div>
          </div>
        ) : (
          <>
            <div className="flex flex-col relative group">
              <label className="text-sm font-semibold text-gray-300 mb-2 flex justify-between">Header <button onClick={()=>copy(header)} className="text-xs text-emerald-500 hover:text-emerald-400 hidden group-hover:block">Copy</button></label>
            ...
            <div className="flex-1 flex flex-col relative group">
              <label className="text-sm font-semibold text-gray-300 mb-2 flex justify-between">Payload <button onClick={()=>copy(payload)} className="text-xs text-purple-500 hover:text-purple-400 hidden group-hover:block">Copy</button></label>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6 h-full">
       <div className="flex flex-col">
        ...
        <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Enter text to hash..." className="w-full min-h-[120px] bg-gray-950 border border-gray-800 rounded-xl p-4 text-gray-200 font-mono text-sm outline-none focus:ring-2 focus:ring-emerald-500" spellCheck={false} />
      </div>
      <div className="flex-1 flex flex-col gap-4">
...
function Base64Tool({ copy }: { copy: (t: string) => void }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full gap-6">
      <div className="flex bg-gray-950 p-1 rounded-lg border border-gray-800 w-fit">
        <button onClick={() => setMode('encode')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${mode === 'encode' ? 'bg-gray-800 text-emerald-400' : 'text-gray-400 hover:text-gray-200'}`}>Encode</button>
        <button onClick={() => setMode('decode')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${mode === 'decode' ? 'bg-gray-800 text-emerald-400' : 'text-gray-400 hover:text-gray-200'}`}>Decode</button>
      </div>
...
function EducationalContent({ activeTab }: { activeTab: TabId }) {
  return (
    <article className="bg-gray-900/50 p-6 sm:p-8 rounded-2xl border border-gray-800/50 text-gray-400 text-sm leading-relaxed">
      {activeTab === 'generate' && (
        <div className="animate-in fade-in duration-500">
          <h2 className="text-xl font-bold text-white mb-4">Application Secrets Architecture</h2>
      ...
      {activeTab === 'decode' && (
        <div className="animate-in fade-in duration-500">
          <h2 className="text-xl font-bold text-white mb-4">JSON Web Token Privacy</h2>
      ...
      {activeTab === 'hash' && (
        <div className="animate-in fade-in duration-500">
          <h2 className="text-xl font-bold text-white mb-4">Cryptographic Hashing</h2>
      ...
      {activeTab === 'base64' && (
        <div className="animate-in fade-in duration-500">
          <h2 className="text-xl font-bold text-white mb-4">Base64 Encoding & Unicode</h2>
